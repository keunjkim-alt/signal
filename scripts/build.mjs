...
const indexPath='dist/index.html',indexHtml=await readFile(indexPath,'utf8'),bundledIndex=indexHtml.replace(/\s*<link rel="stylesheet" href="\.\/[^\"]+" \/>/g,'').replace('</head>',`  <link rel="stylesheet" href="./app.min.css?v=${assetVersion}" />\n</head>`).replace(/<script defer src="\.\/app\.js[^\"]*"><\/script>/,`<script defer src="./app.min.js?v=${assetVersion}"></script>`);

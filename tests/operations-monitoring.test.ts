...
test('client telemetry removes unsafe fields and applies bounded values',()=>{
  const unsafeTag='<scr'+'ipt>',[metric]=normalizeClientMetrics([{type:`api${unsafeTag}`,name:'GET /api/ax/query\nsecret',durationMs:999999,page:`profitability${unsafeTag}`,status:'500',payload:{token:'never'}}]);

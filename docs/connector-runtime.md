...
4. Vercel Hobby 운영 환경에서는 Cron이 매일 06:00(KST)에 예약 소스를 확인한다.
5. 각 소스의 설정 주기가 지난 경우에만 동기화하며, 일중 갱신은 화면의 `지금 동기화`로 즉시 실행한다. 15분·1시간 완전 자동화는 Vercel Pro 또는 별도 스케줄러 도입 시 활성화한다.

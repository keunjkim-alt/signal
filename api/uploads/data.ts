...
import {inferMarketingMapping,marketingControlTotals,validateAndNormalizeMarketing} from '../_lib/marketing.js';
...
import {inventoryLocationsFromRows} from '../_lib/location-master.js';
...
const SUPPORTED=['product_master','inventory_snapshot','sales_order','product_review','marketing_campaign'];
...
export default {async fetch(request:Request){
  ...
  try{
    ...
    sourceId=await ensureDataSource(org,ws,context.user.id,requestedSourceId,entityType);
    if(!template){
      const saved=await ensureConfirmedMappingTemplate(org,ws,context.user.id,entityType,signature,mapping,file.name);
    ...
    const duplicate=await findCompletedUpload(org,ws,entityType,checksum);
    if(duplicate){
    ...
    const upload=await persistRawFile(file,fileBytes,checksum,org,ws,context.user.id,sourceId,entityType);
    rawUploadId=upload.id;
    importSummary={mapping,mappingSource,mappingTemplateId:template?.id||null,period:preview.period};
    const job=(await insert('import_jobs',{organization_id:org,workspace_id:ws,raw_upload_id:upload.id,data_source_id:sourceId,mapping_template_id:template?.id||null,created_by:context.user.id,entity_type:entityType,status:'processing',total_rows:rows.length,started_at:new Date().toISOString(),summary:importSummary}))?.[0];
    importJobId=job.id;
    const sourceControl=entityType==='sales_order'?salesControlTotals(validation.validRows):entityType==='inventory_snapshot'?inventoryControlTotals(validation.validRows):entityType==='marketing_campaign'?marketingControlTotals(validation.validRows):{rows:validation.validRows.length,products:new Set(validation.validRows.map((row:any)=>row.product_code)).size,skus:new Set(validation.validRows.map((row:any)=>row.sku_code)).size},result=entityType==='sales_order'?await ingestSales(validation.validRows,org,ws,sourceId,upload.id,job.id):entityType==='inventory_snapshot'?await ingestInventory(validation.validRows,org,ws,sourceId,upload.id):entityType==='product_review'?await ingestReviews(validation.validRows,org,ws,sourceId,upload.id,job.id):entityType==='marketing_campaign'?await ingestMarketing(validation.validRows,org,ws,sourceId,upload.id,job.id):await ingestProductMaster(validation.validRows,org,ws);
    const allErrors=[...validation.errors.map((item:any)=>({organization_id:org,workspace_id:ws,import_job_id:job.id,...item})),...result.errors.map((item:any)=>({organization_id:org,workspace_id:ws,import_job_id:job.id,...item}))];
    for(const chunk of chunks(allErrors,500))await insert('import_errors',chunk);
    const directlyCounted=['product_master','product_review','marketing_campaign'].includes(entityType),completedAt=new Date().toISOString(),persistedControl=directlyCounted?result.summary.persistedControl:await persistedControlTotals(org,{id:job.id,raw_upload_id:upload.id,entity_type:entityType,workspace_id:ws}),reconciliation=directlyCounted?{entityType,status:'matched',matched:true,checkedAt:completedAt,filename:file.name,jobId:job.id,source:sourceControl,persisted:persistedControl,checks:[{key:'rows',label:'정상 행',unit:'행',source:result.successRows,persisted:result.successRows,difference:0,match:true}]}:buildReconciliation(entityType,sourceControl,persistedControl,{checkedAt:completedAt,filename:file.name,jobId:job.id});
    ...
    try{
      ...
      else if(['product_master','product_review','marketing_campaign'].includes(entityType))analytics={status:'skipped',completed:0,failed:0,total:0,refreshedAt:completedAt,asOfDate:preview.period?.end?.slice?.(0,10)||null,results:[],reason:entityType==='product_review'?'REVIEW_SIGNALS_PRECOMPUTED':entityType==='marketing_campaign'?'MARKETING_METRICS_PRECOMPUTED':'MASTER_DATA_REFRESH_ONLY'};
      else if(status!=='failed'&&result.successRows>0)analytics=await refreshPostImportAnalytics(org,{periodEnd:preview.period?.end,workspaceId:ws});
...
async function ingestInventory(validRows:any[],org:string,ws:string|null,sourceId:string,uploadId:string){
  const skuCodes=[...new Set(validRows.map(row=>row.sku_code))],locationCodes=[...new Set(validRows.map(row=>row.location_code))];
  const locationRows=inventoryLocationsFromRows(validRows).map(row=>({organization_id:org,workspace_id:ws,...row}));
  for(const chunk of chunks(locationRows,500))await insert('locations',chunk,{upsert:true,onConflict:'organization_id,workspace_id,location_code'});
  const skus=await fetchMaster('skus','sku_code',skuCodes,org,ws),locations=await fetchMaster('locations','location_code',locationCodes,org,ws),skuMap=new Map(skus.map((item:any)=>[item.sku_code,item.id])),locationMap=new Map(locations.map((item:any)=>[item.location_code,item.id])),errors:any[]=[],snapshots:any[]=[];
  for(const row of validRows){const skuId=skuMap.get(row.sku_code),locationId=locationMap.get(row.location_code);if(!skuId||!locationId){errors.push({row_number:row.row_number,field_name:!skuId?'sku_code':'location_code',error_code:'MASTER_NOT_FOUND',message:!skuId?`미등록 SKU: ${row.sku_code}`:`미등록 위치: ${row.location_code}`,raw_row:row.raw_row});continue}snapshots.push({organization_id:org,workspace_id:ws,source_id:sourceId,sku_id:skuId,location_id:locationId,snapshot_at:row.snapshot_at,on_hand_qty:row.on_hand_qty,reserved_qty:row.reserved_qty,available_qty:row.available_qty,in_transit_qty:row.in_transit_qty,damaged_qty:row.damaged_qty,safety_stock_qty:row.safety_stock_qty,raw_upload_id:uploadId})}
  for(const chunk of chunks(snapshots,500))await insert('inventory_snapshots',chunk,{upsert:true,onConflict:'organization_id,workspace_id,sku_id,location_id,snapshot_at'});
  return {successRows:snapshots.length,insertedRows:snapshots.length,updatedRows:0,errors,summary:{missingSku:skuCodes.filter(code=>!skuMap.has(code)),missingLocation:locationCodes.filter(code=>!locationMap.has(code)),locationsAutoCreated:locationRows.length,persistedControl:inventoryControlTotals(snapshots)}};
}

async function ingestReviews(validRows:any[],org:string,ws:string|null,sourceId:string,uploadId:string,jobId:string){
  const productCodes=[...new Set(validRows.map(row=>row.product_code))],skuCodes=[...new Set(validRows.map(row=>row.sku_code).filter(Boolean))],[products,skus]=await Promise.all([fetchMaster('products','product_code',productCodes,org,ws),fetchMaster('skus','sku_code',skuCodes,org,ws)]),productMap=new Map(products.map((row:any)=>[row.product_code,row.id])),skuMap=new Map(skus.map((row:any)=>[row.sku_code,row.id])),errors:any[]=[],reviewRows:any[]=[];
  for(const row of validRows){const productId=productMap.get(row.product_code);if(!productId){errors.push({row_number:row.row_number,field_name:'product_code',error_code:'MASTER_NOT_FOUND',message:`미등록 상품: ${row.product_code}`,raw_row:row.raw_row});continue}reviewRows.push({organization_id:org,workspace_id:ws,data_source_id:sourceId,raw_upload_id:uploadId,import_job_id:jobId,product_id:productId,sku_id:skuMap.get(row.sku_code)||null,source_review_id:row.source_review_id,reviewed_at:row.reviewed_at,platform:row.platform,channel_code:row.channel_code,product_code:row.product_code,sku_code:row.sku_code,rating:row.rating,review_text:row.review_text,verified_purchase:row.verified_purchase,helpful_count:row.helpful_count,image_review:row.image_review,customer_token:row.customer_token,order_id:row.order_id,country_code:row.country_code,color:row.color,size:row.size,seller_response_status:row.seller_response_status,updated_at:new Date().toISOString()})}
  for(const chunk of chunks(reviewRows,400))await insert('product_reviews',chunk,{upsert:true,onConflict:'organization_id,workspace_id,platform,source_review_id'});
  ...
  const importedReviewIds=reviewIds.map(row=>row.id);for(const group of chunks(importedReviewIds,200)){const filter=`in.(${group.join(',')})`;await supabase(`/rest/v1/review_aspect_signals?organization_id=eq.${encodeURIComponent(org)}&${ws?`workspace_id=eq.${encodeURIComponent(ws)}&`:''}review_id=${encodeURIComponent(filter)}`,{serviceRole:true,method:'DELETE'})}
  const idMap=new Map(reviewIds.map(row=>[`${row.platform}:${row.source_review_id}`,row.id])),signals:any[]=[];for(const row of validRows){const reviewId=idMap.get(`${row.platform}:${row.source_review_id}`);if(!reviewId)continue;for(const signal of classifyReview(row))signals.push({organization_id:org,workspace_id:ws,review_id:reviewId,...signal})}
...
async function ingestMarketing(validRows:any[],org:string,ws:string|null,sourceId:string,uploadId:string,jobId:string){
  const now=new Date().toISOString(),campaignRows=dedupe(validRows.map(row=>({organization_id:org,workspace_id:ws,data_source_id:sourceId,raw_upload_id:uploadId,import_job_id:jobId,source_campaign_id:row.source_campaign_id,campaign_name:row.campaign_name,start_at:row.start_at,end_at:row.end_at,channel_code:row.channel_code,campaign_type:row.campaign_type,budget:row.budget,owner:row.owner,status:row.status,updated_at:now})),'source_campaign_id');
  const existing=await fetchMaster('marketing_campaigns','source_campaign_id',campaignRows.map(row=>row.source_campaign_id),org,ws),existingIds=new Set(existing.map((row:any)=>row.source_campaign_id));
  for(const chunk of chunks(campaignRows,400))await insert('marketing_campaigns',chunk,{upsert:true,onConflict:'organization_id,workspace_id,source_campaign_id'});
  const campaigns=await fetchMaster('marketing_campaigns','source_campaign_id',campaignRows.map(row=>row.source_campaign_id),org,ws),campaignMap=new Map(campaigns.map((row:any)=>[row.source_campaign_id,row.id])),errors:any[]=[],snapshots:any[]=[];
  for(const row of validRows){const campaignId=campaignMap.get(row.source_campaign_id);if(!campaignId){errors.push({row_number:row.row_number,field_name:'campaign_id',error_code:'CAMPAIGN_NOT_FOUND',message:`캠페인 저장 실패: ${row.source_campaign_id}`,raw_row:row.raw_row});continue}snapshots.push({organization_id:org,workspace_id:ws,campaign_id:campaignId,data_source_id:sourceId,raw_upload_id:uploadId,import_job_id:jobId,observed_at:row.observed_at,spend:row.spend,impressions:row.impressions,clicks:row.clicks,conversions:row.conversions,attributed_sales:row.attributed_sales,control_group_size:row.control_group_size,control_group_conversions:row.control_group_conversions})}
  for(const chunk of chunks(snapshots,500))await insert('marketing_campaign_snapshots',chunk,{upsert:true,onConflict:'organization_id,workspace_id,campaign_id,observed_at'});
  const insertedCampaigns=campaignRows.filter(row=>!existingIds.has(row.source_campaign_id)).length,persistedControl=marketingControlTotals(validRows.filter(row=>campaignMap.has(row.source_campaign_id)));
  return {successRows:snapshots.length,insertedRows:snapshots.length+insertedCampaigns,updatedRows:campaignRows.length-insertedCampaigns,errors,summary:{campaigns:campaignRows.length,snapshots:snapshots.length,persistedControl}};
}

async function ingestProductMaster(validRows:any[],org:string,ws:string|null){
  const productRows=dedupe(validRows.map(row=>({organization_id:org,workspace_id:ws,product_code:row.product_code,product_name:row.product_name,category_l1:row.category_l1||null,category_l2:row.category_l2||null,season:row.season||null,image_url:row.image_url||null,attributes:{list_price:row.list_price,unit_cost:row.unit_cost,created_from:'product_master'}})),'product_code');
  const existingProducts=await fetchMaster('products','product_code',productRows.map(row=>row.product_code),org,ws),existingProductCodes=new Set(existingProducts.map((row:any)=>row.product_code));
  for(const chunk of chunks(productRows,500))await insert('products',chunk,{upsert:true,onConflict:'organization_id,workspace_id,product_code'});
  const products=await fetchMaster('products','product_code',productRows.map(row=>row.product_code),org,ws),productMap=new Map(products.map((item:any)=>[item.product_code,item.id]));
  const skuRows=dedupe(validRows.map(row=>({organization_id:org,workspace_id:ws,product_id:productMap.get(row.product_code)||null,sku_code:row.sku_code,barcode:row.barcode||null,color:row.color||null,size:row.size||null,external_codes:{created_from:'product_master'}})),'sku_code'),existingSkus=await fetchMaster('skus','sku_code',skuRows.map(row=>row.sku_code),org,ws),existingSkuCodes=new Set(existingSkus.map((row:any)=>row.sku_code));
  for(const chunk of chunks(skuRows,500))await insert('skus',chunk,{upsert:true,onConflict:'organization_id,workspace_id,sku_code'});
...
async function ingestSales(validRows:any[],org:string,ws:string|null,sourceId:string,uploadId:string,jobId:string){
  ...
  const productRows=dedupe(validRows.map(row=>({organization_id:org,workspace_id:ws,product_code:row.sku_code,product_name:row.product_name||row.sku_code,category_l1:row.category||null,attributes:{created_from:'sales_file'}})),'product_code');
  for(const chunk of chunks(productRows,500))await insert('products',chunk,{upsert:true,onConflict:'organization_id,workspace_id,product_code'});
  const products=await fetchMaster('products','product_code',skuCodes,org,ws),productMap=new Map(products.map((item:any)=>[item.product_code,item.id]));
  const skuRows=skuCodes.map(code=>({organization_id:org,workspace_id:ws,product_id:productMap.get(code)||null,sku_code:code,external_codes:{created_from:'sales_file'}}));
  for(const chunk of chunks(skuRows,500))await insert('skus',chunk,{upsert:true,onConflict:'organization_id,workspace_id,sku_code'});
  for(const chunk of chunks(locationRows.map(row=>({organization_id:org,workspace_id:ws,...row,location_type:'store',timezone:'Asia/Seoul',active:true})),500))await insert('locations',chunk,{upsert:true,onConflict:'organization_id,workspace_id,location_code'});
  const [skus,locations]=await Promise.all([fetchMaster('skus','sku_code',skuCodes,org,ws),fetchMaster('locations','location_code',locationRows.map(row=>row.location_code),org,ws)]),skuMap=new Map(skus.map((item:any)=>[item.sku_code,item.id])),locationMap=new Map(locations.map((item:any)=>[item.location_code,item.id]));
...
async function ensureDataSource(org:string,ws:string|null,userId:string,requestedId:string,entityType:string){
  ...
  const provider=`file_upload_${entityType}`,query=new URLSearchParams({organization_id:`eq.${org}`,provider:`eq.${provider}`,select:'id',limit:'1'});if(ws)query.set('workspace_id',`eq.${ws}`);const existing=(await supabase(`/rest/v1/data_sources?${query}`,{serviceRole:true})).data||[];
  if(existing[0])return existing[0].id;
  return (await insert('data_sources',{organization_id:org,workspace_id:ws,source_type:'file',provider,name:entityType==='sales_order'?'판매 파일 업로드':entityType==='inventory_snapshot'?'재고 파일 업로드':entityType==='product_review'?'리뷰·VOC 파일 업로드':entityType==='marketing_campaign'?'캠페인·성과 파일 업로드':'상품 마스터 파일 업로드',status:'active',data_mode:'connected',sync_mode:'manual',config:{entity_type:entityType},created_by:userId}))?.[0]?.id;
}

async function persistRawFile(file:File,fileBytes:ArrayBuffer,checksum:string,org:string,ws:string|null,userId:string,sourceId:string,entityType:string){
  ...
  return (await insert('raw_uploads',{id:uploadId,organization_id:org,workspace_id:ws,data_source_id:sourceId,uploaded_by:userId,original_filename:file.name,storage_path:storagePath,content_type:file.type||null,byte_size:file.size,checksum,entity_type:entityType,status:'processing'}))?.[0];
}
...
async function findCompletedUpload(org:string,ws:string|null,entityType:string,checksum:string){const query=new URLSearchParams({organization_id:`eq.${org}`,entity_type:`eq.${entityType}`,checksum:`eq.${checksum}`,status:'eq.completed',select:'id,created_at,original_filename',order:'created_at.desc',limit:'5'});if(ws)query.set('workspace_id',`eq.${ws}`);const uploads=((await supabase(`/rest/v1/raw_uploads?${query}`,{serviceRole:true})).data||[]);for(const upload of uploads){const job=await findImportJob(upload.id);if(job?.status==='completed')return upload}return null}
async function findMappingTemplate(org:string,ws:string|null,entityType:string,signature:string,requestedSourceId:string|null){
  if(requestedSourceId){const specific=new URLSearchParams({organization_id:`eq.${org}`,entity_type:`eq.${entityType}`,header_signature:`eq.${signature}`,data_source_id:`eq.${requestedSourceId}`,active:'eq.true',select:'id,name,version,mapping,data_source_id',order:'version.desc,created_at.desc',limit:'1'});if(ws)specific.set('workspace_id',`eq.${ws}`);const row=((await supabase(`/rest/v1/mapping_templates?${specific}`,{serviceRole:true})).data||[])[0];if(row)return row}
  const generic=new URLSearchParams({organization_id:`eq.${org}`,entity_type:`eq.${entityType}`,header_signature:`eq.${signature}`,data_source_id:'is.null',active:'eq.true',select:'id,name,version,mapping,data_source_id',order:'version.desc,created_at.desc',limit:'1'});if(ws)generic.set('workspace_id',`eq.${ws}`);
  return ((await supabase(`/rest/v1/mapping_templates?${generic}`,{serviceRole:true})).data||[])[0]||null;
}
async function ensureConfirmedMappingTemplate(org:string,ws:string|null,userId:string,entityType:string,signature:string,mapping:any,filename:string){
  const existing=await findMappingTemplate(org,ws,entityType,signature,null);if(existing)return {template:existing,created:false};
  const base=String(filename||'회사 데이터').replace(/\.[^.]+$/,'').slice(0,60),template=(await insert('mapping_templates',{organization_id:org,workspace_id:ws,data_source_id:null,name:`${base} · 확인된 매핑`,entity_type:entityType,header_signature:signature,mapping,transformations:{source:'confirmed_import'},version:1,active:true,created_by:userId}))?.[0];
...
function inferEntityMapping(entityType:string,headers:string[]){return entityType==='sales_order'?inferSalesMapping(headers):entityType==='inventory_snapshot'?inferMapping(headers):entityType==='product_review'?inferReviewMapping(headers):entityType==='marketing_campaign'?inferMarketingMapping(headers):inferProductMapping(headers)}
function validateEntityRows(entityType:string,rows:Record<string,any>[],mapping:any){return entityType==='sales_order'?validateAndNormalizeSales(rows,mapping):entityType==='inventory_snapshot'?validateAndNormalize(rows,mapping):entityType==='product_review'?validateAndNormalizeReviews(rows,mapping):entityType==='marketing_campaign'?validateAndNormalizeMarketing(rows,mapping):validateAndNormalizeProducts(rows,mapping)}
function inferPeriod(rows:any[],entityType:string){const field=entityType==='sales_order'?'sold_at':entityType==='inventory_snapshot'?'snapshot_at':entityType==='product_review'?'reviewed_at':entityType==='marketing_campaign'?'observed_at':null,values=field?rows.map(row=>row[field]).filter(Boolean).sort():[];return {start:values[0]||null,end:values.at(-1)||null}}

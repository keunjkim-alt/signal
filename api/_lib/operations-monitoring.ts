...
export function summarizeOperationsMonitoring(input:any={}){
  ...
  const api=telemetry.filter(row=>row.type==='api'),navigation=telemetry.filter(row=>row.type==='navigation'||row.type==='render'),webVitals=telemetry.filter(row=>row.type==='webvital'),longTasks=telemetry.filter(row=>row.type==='longtask'),ax=api.filter(row=>row.name.includes('/api/ax/query'));
  ...
  return {
    ...
    summary:{apiRequests:api.length,apiAverageMs:average(api.map(row=>row.durationMs)),apiP95Ms:percentile(api.map(row=>row.durationMs),.95),apiErrors:apiErrors.length,slowApiRequests:slowApi.length,navigationAverageMs:average(navigation.map(row=>row.durationMs)),lcpP75Ms:percentile(webVitals.filter(row=>row.name==='lcp').map(row=>row.durationMs),.75),fcpP75Ms:percentile(webVitals.filter(row=>row.name==='fcp').map(row=>row.durationMs),.75),longTaskCount:longTasks.length,longTaskP95Ms:percentile(longTasks.map(row=>row.durationMs),.95),axRequests:ax.length,axAverageMs:average(ax.map(row=>row.durationMs)),axP95Ms:percentile(ax.map(row=>row.durationMs),.95),axFallbacks:fallbacks.length,axMessages:assistantMessages.length,cacheHits:queryCache.reduce((sum:number,row:any)=>sum+Number(row.hit_count||0),0),sourceTotal:sources.length,sourceErrors:sourceErrors.length,importTotal:jobs.length,importFailed:failedJobs.length,importPartial:partialJobs.length,analyticsFailed:failedRuns.length},
    attention,endpoints,

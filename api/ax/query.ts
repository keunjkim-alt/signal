...
export default {async fetch(request:Request){
  ...
  try{const body=await bodyJson(request),page=String(body?.page||'hub'),context=await requestContext(request,{includeProfile:false,includeBrands:false,permissionPage:page}),org=context.membership.organization_id,contextReady=performance.now();
    ...
    const persisted=[insert('ax_messages',{organization_id:org,conversation_id:conversationId,user_id:context.user.id,role:'user',content:question,page_key:page,source:'user'}),insert('ax_messages',{organization_id:org,conversation_id:conversationId,user_id:context.user.id,role:'assistant',content:answer,page_key:page,query_spec:queryResult.spec,visualization_spec:visualization,model:plan.model||null,source:plan.source||'heuristic'}),update('ax_conversations',{id:`eq.${conversationId}`,organization_id:`eq.${org}`,user_id:`eq.${context.user.id}`},{page_key:page,last_message_at:new Date().toISOString(),updated_at:new Date().toISOString()})];

...
export default {async fetch(request:Request){
  ...
  try{const body=await bodyJson(request),page=String(body?.page||'hub'),context=await requestContext(request,{includeProfile:false,includeBrands:false,permissionPage:page}),org=context.membership.organization_id,contextReady=performance.now();
    ...
    const planReady=performance.now(),conversationResult=await conversationTask;if(conversationResult.error)throw conversationResult.error;const conversationId=conversationResult.id as string;
    ...
    const [,queryResult]=await Promise.all([insert('ax_messages',{organization_id:org,conversation_id:conversationId,user_id:context.user.id,role:'user',content:question,page_key:page,source:'user'}),queryTask]),queryReady=performance.now();
    ...
    await Promise.all(persisted);const completed=performance.now(),timing={totalMs:Math.round(completed-requestStarted),contextMs:Math.round(contextReady-requestStarted),planMs:Math.round(planReady-planStarted),queryMs:Math.round(queryReady-planReady),persistMs:Math.round(completed-queryReady),route:plan.source||'heuristic',cacheHit:Boolean(cache)};
    return json({ok:true,conversationId,question,answer,plan,query:queryResult.spec,data:queryResult.data,warning:runtimeWarning,visualization,timing},200,{'server-timing':`context;dur=${timing.contextMs}, plan;dur=${timing.planMs}, query;dur=${timing.queryMs}, persist;dur=${timing.persistMs}`})}catch(error:any){return errorResponse(error,error.status||500)}
}};

...
export default {async fetch(request:Request){
  ...
  try{const context=await requestContext(request);const body=await bodyJson(request),page=String(body?.page||'hub'),org=context.membership.organization_id;
    ...
    const conversationResult=await conversationTask;if(conversationResult.error)throw conversationResult.error;const conversationId=conversationResult.id as string;
    ...
    return json({ok:true,conversationId,question,answer,plan,query:queryResult.spec,data:queryResult.data,warning:runtimeWarning,visualization})}catch(error:any){return errorResponse(error,error.status||500)}
}};

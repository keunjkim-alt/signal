...
import {customerReturnInsights,marketingInsights,productionWorkflow,reviewInsights,runDashboardQuery} from '../dashboards/query.js';
...
const conversationMemory=new Map<string,{state:any;expiresAt:number}>();
const CONVERSATION_MEMORY_TTL=20_000,MAX_CONVERSATION_MEMORY=160;
function conversationMemoryKey(context:any,conversationId:string,org:string,ws:string|null){return `${org}:${ws||'legacy'}:${context.user.id}:${conversationId}`}
function rememberConversationState(key:string,state:any){
  const now=Date.now();conversationMemory.set(key,{state,expiresAt:now+CONVERSATION_MEMORY_TTL});
  for(const [candidate,entry] of conversationMemory)if(entry.expiresAt<=now)conversationMemory.delete(candidate);
  if(conversationMemory.size>MAX_CONVERSATION_MEMORY)for(const candidate of [...conversationMemory.keys()].slice(0,conversationMemory.size-MAX_CONVERSATION_MEMORY))conversationMemory.delete(candidate);
}
...
function rowsForRequestedProduct(rows:any[],question:string){
  ...
  const matching=rows.filter(row=>[row?.product_code,row?.product_name,row?.source_campaign_id,row?.campaign_name].some(value=>{
...
async function productIntelligence(context:any,page:string,mode:'matching'|'forecast'|'discount'|'review'|'customer'|'returns'|'production'|'marketing',question=''){
  if(mode==='marketing'){
    const data=await marketingInsights(context),rows=rowsForRequestedProduct(data.campaigns||[],question);
    return {spec:{source:'precomputed_marketing_metrics',mode,visualization:'table'},data:{rows,summary:data.summary,channels:data.channels,trend:data.trend,actions:data.actions,source:'precomputed_marketing_metrics',note:'업로드된 캠페인 성과 스냅샷에서 예산·기여매출·ROAS·대조군 효과를 계산했습니다.'}};
  }
  if(mode==='production'){
...
async function prepareConversationState(context:any,body:any,page:string,org:string,ws:string|null){
  const requested=String(body?.conversationId||'');
  if(!requested){
    const conversationId=randomUUID(),state={context:sanitizeAxContext(null),messages:[]},memoryKey=conversationMemoryKey(context,conversationId,org,ws);
    rememberConversationState(memoryKey,state);
    const conversationCreate=insert('ax_conversations',{id:conversationId,organization_id:org,workspace_id:ws,user_id:context.user.id,title:String(body?.question||'').slice(0,80),page_key:page,status:'active'});
    return {conversationId,state,conversationCreate,memoryKey};
  }
  const memoryKey=conversationMemoryKey(context,requested,org,ws),memory=conversationMemory.get(memoryKey);
  if(memory&&memory.expiresAt>Date.now()){memory.expiresAt=Date.now()+CONVERSATION_MEMORY_TTL;return {conversationId:requested,state:memory.state,conversationCreate:null,memoryKey}}
  ...
  rememberConversationState(memoryKey,state);
  return {conversationId:requested,state,conversationCreate:null,memoryKey};
}
...
const queryHandler={async fetch(request:Request){
  ...
  try{const body=await bodyJson(request),page=String(body?.page||'hub'),context=await requestContext(request,{includeProfile:false,includeBrands:false,permissionPage:page}),org=context.membership.organization_id,ws=workspaceId(context),contextReady=performance.now();
    ...
    const planStarted=performance.now(),filters=body?.filters||{},[prepared,watermark]=await Promise.all([prepareConversationState(context,body,page,org,ws),latestWatermark(org,ws)]),conversationId=prepared.conversationId,conversationState=prepared.state,summaryRequested=isConversationSummaryIntent(question),conversationSummary=summaryRequested?buildConversationSummary(conversationState.context,conversationState.messages):null,mode=summaryRequested?null:inheritedIntelligenceMode(intelligenceMode(question,page),question,conversationState.context),normalized=normalizeQuestion(question),modelContext=modelConversationContext(conversationState.context,conversationState.messages),useModel=!summaryRequested&&!mode&&requiresOpenAI(question),contextFingerprint=stable({version:conversationState.context.version,summary:conversationState.context.summary,filters:conversationState.context.filters,subjects:conversationState.context.subjects}),useCache=useModel,cacheKey=useCache?createHash('sha256').update(stable({cacheVersion:'intelligence-v8',workspaceId:ws,page,question:normalized,filters,contextFingerprint,watermark})).digest('hex'):'';let cache=useCache?await cachedPlan(org,ws,cacheKey):null,plan:any,cacheTouch:Promise<any>|null=null;
    if(summaryRequested)plan={...(conversationState.context.lastQuerySpec||{}),metric:conversationState.context.metric||'net_sales',dimension:conversationState.context.dimension||'product',visualization:'table',periodDays:conversationState.context.periodDays||14,filters:conversationState.context.filters||filters,limit:3,title:'지금까지 분석 요약',explanation:'현재 대화의 분석 범위, 핵심 대상과 다음 판단을 세 줄로 정리했습니다.',source:'conversation_summary',model:null};else if(cache){plan={...cache.plan_spec,source:'cache',model:cache.model||null};cacheTouch=update('ax_query_cache',{id:`eq.${cache.id}`},{hit_count:Number(cache.hit_count||0)+1,updated_at:new Date().toISOString()})}else if(mode==='marketing')plan={metric:'marketing',dimension:'campaign',visualization:'table',periodDays:31,filters,title:'캠페인별 예산·매출 기여·효율',explanation:'저장된 캠페인 성과 스냅샷에서 예산 집행, 기여매출, ROAS와 대조군 효과를 조회했습니다.',source:'precomputed_marketing_metrics',model:null};else if(mode==='production')plan={metric:'production',dimension:'production_order',visualization:'table',periodDays:28,filters,title:'생산오더 납기·공정 실행 현황',explanation:'승인된 재주문에서 생성된 실제 생산오더와 현재 공정·납기 위험을 조회했습니다.',source:'precomputed_operational_workflow',model:null};else if(mode==='discount')plan={metric:'discount',dimension:'product',visualization:'curve',periodDays:14,filters,title:'상품·채널별 최적 할인 추천',explanation:'저장된 가격 탄력성·기여이익·재고 위험 시나리오에서 최적 할인값을 조회했습니다.',source:'precomputed_intelligence',model:null};else if(mode==='forecast')plan={metric:'forecast',dimension:'product',visualization:'bar',periodDays:14,filters,title:'제품별 14일 수요예측과 재고 커버',explanation:'저장된 판매속도·재고 스냅샷에서 예측 판매량과 재주문 필요 수량을 조회했습니다.',source:'precomputed_intelligence',model:null};else if(mode==='matching')plan={metric:'matching',dimension:'product',visualization:'table',periodDays:1,filters,title:'자사 상품과 외부 경쟁 상품 매칭',explanation:'스타일·가격·순위·리뷰 특징으로 미리 계산한 매칭 추천을 조회했습니다.',source:'precomputed_intelligence',model:null};else if(mode==='review')plan={metric:'review_signal',dimension:'product',visualization:'table',periodDays:90,filters,title:'상품별 리뷰·VOC 위험 신호',explanation:'업로드 시 저장한 리뷰 속성·감정·반품 위험 신호를 조회했습니다.',source:'precomputed_review_signals',model:null};else if(mode==='customer')plan={metric:'customer',dimension:'location',visualization:'table',periodDays:90,filters,title:'고객·지역 재구매 신호',explanation:'저장된 익명 고객 토큰과 시군구 배송지역 집계를 조회했습니다.',source:'precomputed_operational_insight',model:null};else if(mode==='returns')plan={metric:'returns',dimension:'product',visualization:'bar',periodDays:90,filters,title:'반품·취소 위험 신호',explanation:'저장된 주문·반품 수량·취소 상태에서 제품별 위험을 조회했습니다.',source:'precomputed_operational_insight',model:null};else plan=await createAnalysisPlan(question,page,filters,{useModel,conversationContext:modelContext,recentMessages:modelContext.recentMessages});
    const resolution=resolveAxContextPlan({previous:conversationState.context,question,page,filters,plan}),resolvedPlan=resolution.plan,planReady=performance.now(),queryTask=(async()=>{try{return summaryRequested?{spec:{source:'conversation_summary',visualization:'summary'},data:{rows:conversationSummary?.lines||[],summaryLines:conversationSummary?.lines||[],source:'conversation_summary'}}:cache?.result_spec?cache.result_spec:mode?await productIntelligence(context,page,mode,`${question} ${resolvedPlan.filters?.product||''}`):await runDashboardQuery(context,{...resolvedPlan,page})}catch(error:any){return {spec:resolvedPlan,data:{rows:[]},warning:error.message}}})(),queryResult=await queryTask,queryReady=performance.now(),finalContext=summaryRequested?{...resolution.context,lastResultSummary:conversationState.context.lastResultSummary}:finalizeAxContext(resolution.context,queryResult.data,watermark);
    const modeDimension=mode==='customer'?'location':mode==='production'?'production_order':mode==='marketing'?'campaign':mode?'product':resolvedPlan.dimension,runtimeWarning=queryResult.warning||resolvedPlan.warning||null,visualization={type:summaryRequested?'summary':['matching','review','customer','production','marketing'].includes(String(mode))?'table':mode==='forecast'||mode==='returns'?'bar':mode==='discount'?'curve':resolvedPlan.visualization,title:resolvedPlan.title,explanation:resolvedPlan.explanation,metric:mode||resolvedPlan.metric,dimension:modeDimension,rows:queryResult.data?.rows||[]},inheritance=resolution.inherited.length?` 이전 분석의 ${resolution.inherited.join(', ')} 조건을 이어받았습니다.`:'',answer=summaryRequested?conversationSummary!.answer:queryResult.warning?`${resolvedPlan.explanation}${inheritance} 현재 조건에서는 일부 데이터 조회가 제한되었습니다.`:`${resolvedPlan.explanation}${inheritance} ${visualization.rows.length}개 결과를 현재 권한 범위에서 조회했습니다.${queryResult.data?.note?` ${queryResult.data.note}`:''}`;
    const userCreatedAt=new Date().toISOString(),assistantCreatedAt=new Date(Date.now()+1).toISOString(),conversationFilter:any={id:`eq.${conversationId}`,organization_id:`eq.${org}`,user_id:`eq.${context.user.id}`};if(ws)conversationFilter.workspace_id=`eq.${ws}`;
    const recentMessages=[...(conversationState.messages||[]),{role:'user',content:question,page_key:page,created_at:userCreatedAt},{role:'assistant',content:answer,page_key:page,created_at:assistantCreatedAt}].slice(-8);rememberConversationState(prepared.memoryKey,{context:finalContext,messages:recentMessages});
    const persistConversation=async()=>{if(prepared.conversationCreate)await prepared.conversationCreate;const persisted=[saveConversationContext(context,conversationId,finalContext,org,ws),insert('ax_messages',{organization_id:org,workspace_id:ws,conversation_id:conversationId,user_id:context.user.id,role:'user',content:question,page_key:page,source:'user',created_at:userCreatedAt}),insert('ax_messages',{organization_id:org,workspace_id:ws,conversation_id:conversationId,user_id:context.user.id,role:'assistant',content:answer,page_key:page,query_spec:{...queryResult.spec,resolvedContext:finalContext,contextChanges:resolution.changes,inherited:resolution.inherited},visualization_spec:visualization,model:resolvedPlan.model||null,source:resolvedPlan.source||'heuristic',created_at:assistantCreatedAt}),update('ax_conversations',conversationFilter,{page_key:page,last_message_at:assistantCreatedAt,updated_at:assistantCreatedAt})];if(cacheTouch)persisted.push(cacheTouch);if(useCache&&!cache)persisted.push(insert('ax_query_cache',{organization_id:org,workspace_id:ws,cache_key:cacheKey,page_key:page,question_normalized:normalized,filters:resolvedPlan.filters,plan_spec:resolvedPlan,result_spec:queryResult,data_watermark:watermark,model:resolvedPlan.model||null,token_usage:resolvedPlan.usage||null,expires_at:new Date(Date.now()+15*60*1000).toISOString()},{upsert:true,onConflict:'organization_id,cache_key'}));if(resolvedPlan.degraded)persisted.push(audit(context,'ax.router_fallback','ax_conversation',conversationId,{page,warningCode:resolvedPlan.warningCode||'AX_DEGRADED',source:resolvedPlan.source}));await Promise.all(persisted)};

...
import {runDashboardQuery} from '../dashboards/query.js';
...
async function productIntelligence(context:any,page:string,mode:'matching'|'forecast'|'discount',question=''){
  if(mode==='discount'){
...
export default {async fetch(request:Request){
  ...
  try{const context=await requestContext(request);const body=await bodyJson(request),page=String(body?.page||'hub'),org=context.membership.organization_id;
    ...
    const visualization={type:mode==='matching'?'table':mode==='forecast'?'bar':mode==='discount'?'curve':plan.visualization,title:plan.title,explanation:plan.explanation,metric:mode||plan.metric,dimension:mode?'product':plan.dimension,rows:queryResult.data?.rows||[]},answer=queryResult.warning?`${plan.explanation} 현재 조건에서는 일부 데이터 조회가 제한되었습니다.`:`${plan.explanation} ${visualization.rows.length}개 결과를 현재 권한 범위에서 조회했습니다.${queryResult.data?.note?` ${queryResult.data.note}`:''}`;

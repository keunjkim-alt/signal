...
async function latestWatermark(org:string){
  const memory=watermarkMemory.get(org);
  ...
  watermarkMemory.set(org,{value,expiresAt:Date.now()+30_000});
  return value;
}
...
async function resolveConversationId(context:any,body:any,page:string,org:string){
  ...
  if(requested){const check=new URLSearchParams({id:`eq.${requested}`,organization_id:`eq.${org}`,user_id:`eq.${context.user.id}`,select:'id'});if(!((await supabase(`/rest/v1/ax_conversations?${check}`,{token:context.accessToken})).data||[]).length)throw Object.assign(new Error('Conversation not found'),{status:404});return requested}
  const created=(await insert('ax_conversations',{organization_id:org,user_id:context.user.id,title:String(body?.question||'').slice(0,80),page_key:page,status:'active'}))?.[0];
  return created.id;
}
...
export default {async fetch(request:Request){
  ...
  try{const body=await bodyJson(request),page=String(body?.page||'hub'),context=await requestContext(request,{includeProfile:false,includeBrands:false,permissionPage:page}),org=context.membership.organization_id,contextReady=performance.now();
    ...
    const planStarted=performance.now(),conversationTask=resolveConversationId(context,body,page,org).then(id=>({id,error:null})).catch(error=>({id:null,error})),filters=body?.filters||{},mode=intelligenceMode(question,page),normalized=normalizeQuestion(question),useModel=!mode&&requiresOpenAI(question),useCache=useModel,watermark=useCache?await latestWatermark(org):null,cacheKey=useCache?createHash('sha256').update(stable({cacheVersion:'intelligence-v5',page,question:normalized,filters,watermark})).digest('hex'):'';let cache=useCache?await cachedPlan(org,cacheKey):null,plan:any,cacheTouch:Promise<any>|null=null;
    if(cache){plan={...cache.plan_spec,source:'cache',model:cache.model||null};cacheTouch=update('ax_query_cache',{id:`eq.${cache.id}`},{hit_count:Number(cache.hit_count||0)+1,updated_at:new Date().toISOString()})}else if(mode==='production')plan={metric:'production',dimension:'production_order',visualization:'table',periodDays:28,filters,title:'생산오더 납기·공정 실행 현황',explanation:'승인된 재주문에서 생성된 실제 생산오더와 현재 공정·납기 위험을 조회했습니다.',source:'precomputed_operational_workflow',model:null};else if(mode==='discount')plan={metric:'discount',dimension:'product',visualization:'curve',periodDays:14,filters,title:'상품·채널별 최적 할인 추천',explanation:'저장된 가격 탄력성·기여이익·재고 위험 시나리오에서 최적 할인값을 조회했습니다.',source:'precomputed_intelligence',model:null};else if(mode==='forecast')plan={metric:'forecast',dimension:'product',visualization:'bar',periodDays:14,filters,title:'제품별 14일 수요예측과 재고 커버',explanation:'저장된 판매속도·재고 스냅샷에서 예측 판매량과 재주문 필요 수량을 조회했습니다.',source:'precomputed_intelligence',model:null};else if(mode==='matching')plan={metric:'matching',dimension:'product',visualization:'table',periodDays:1,filters,title:'자사 상품과 외부 경쟁 상품 매칭',explanation:'스타일·가격·순위·리뷰 특징으로 미리 계산한 매칭 추천을 조회했습니다.',source:'precomputed_intelligence',model:null};else if(mode==='review')plan={metric:'review_signal',dimension:'product',visualization:'table',periodDays:90,filters,title:'상품별 리뷰·VOC 위험 신호',explanation:'업로드 시 저장한 리뷰 속성·감정·반품 위험 신호를 조회했습니다.',source:'precomputed_review_signals',model:null};else if(mode==='customer')plan={metric:'orders',dimension:'location',visualization:'table',periodDays:90,filters,title:'고객·지역 재구매 신호',explanation:'저장된 익명 고객 토큰과 시군구 배송지역 집계를 조회했습니다.',source:'precomputed_operational_insight',model:null};else if(mode==='returns')plan={metric:'return_rate',dimension:'product',visualization:'bar',periodDays:90,filters,title:'반품·취소 위험 신호',explanation:'저장된 주문·반품 수량·취소 상태에서 제품별 위험을 조회했습니다.',source:'precomputed_operational_insight',model:null};else plan=await createAnalysisPlan(question,page,filters,{useModel});
    const planReady=performance.now(),queryTask=(async()=>{try{return cache?.result_spec?cache.result_spec:mode?await productIntelligence(context,page,mode,question):await runDashboardQuery(context,{...plan,page,filters:{...plan.filters,...filters}})}catch(error:any){return {spec:plan,data:{rows:[]},warning:error.message}}})(),[conversationResult,queryResult]=await Promise.all([conversationTask,queryTask]),queryReady=performance.now();if(conversationResult.error)throw conversationResult.error;const conversationId=conversationResult.id as string;
    const modeDimension=mode==='customer'?'location':mode==='production'?'production_order':mode?'product':plan.dimension,runtimeWarning=queryResult.warning||plan.warning||null,visualization={type:['matching','review','customer','production'].includes(String(mode))?'table':mode==='forecast'||mode==='returns'?'bar':mode==='discount'?'curve':plan.visualization,title:plan.title,explanation:plan.explanation,metric:mode||plan.metric,dimension:modeDimension,rows:queryResult.data?.rows||[]},answer=queryResult.warning?`${plan.explanation} 현재 조건에서는 일부 데이터 조회가 제한되었습니다.`:`${plan.explanation} ${visualization.rows.length}개 결과를 현재 권한 범위에서 조회했습니다.${queryResult.data?.note?` ${queryResult.data.note}`:''}`;
    const userCreatedAt=new Date().toISOString(),assistantCreatedAt=new Date(Date.now()+1).toISOString(),persisted=[insert('ax_messages',{organization_id:org,conversation_id:conversationId,user_id:context.user.id,role:'user',content:question,page_key:page,source:'user',created_at:userCreatedAt}),insert('ax_messages',{organization_id:org,conversation_id:conversationId,user_id:context.user.id,role:'assistant',content:answer,page_key:page,query_spec:queryResult.spec,visualization_spec:visualization,model:plan.model||null,source:plan.source||'heuristic',created_at:assistantCreatedAt}),update('ax_conversations',{id:`eq.${conversationId}`,organization_id:`eq.${org}`,user_id:`eq.${context.user.id}`},{page_key:page,last_message_at:assistantCreatedAt,updated_at:assistantCreatedAt})];
    ...
    if(plan.degraded)persisted.push(audit(context,'ax.router_fallback','ax_conversation',conversationId,{page,warningCode:plan.warningCode||'AX_DEGRADED',source:plan.source}));
    waitUntil(Promise.all(persisted).then(()=>undefined).catch(error=>console.error('[AX persistence]',error?.message||error)));const completed=performance.now(),timing={totalMs:Math.round(completed-requestStarted),contextMs:Math.round(contextReady-requestStarted),planMs:Math.round(planReady-planStarted),queryMs:Math.round(queryReady-planReady),persistQueued:true,route:plan.source||'heuristic',cacheHit:Boolean(cache)};
    return json({ok:true,conversationId,question,answer,plan,query:queryResult.spec,data:queryResult.data,warning:runtimeWarning,visualization,timing},200,{'server-timing':`context;dur=${timing.contextMs}, plan;dur=${timing.planMs}, query;dur=${timing.queryMs}`})}catch(error:any){return errorResponse(error,error.status||500)}
}};

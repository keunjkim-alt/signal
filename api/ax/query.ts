...
import {customerReturnInsights,productionWorkflow,runDashboardQuery} from '../dashboards/query.js';
...
async function productIntelligence(context:any,page:string,mode:'matching'|'forecast'|'discount'|'customer'|'returns'|'production',question=''){
  if(mode==='production'){
...
export default {async fetch(request:Request){
  ...
  try{const body=await bodyJson(request),page=String(body?.page||'hub'),context=await requestContext(request,{includeProfile:false,includeBrands:false,permissionPage:page}),org=context.membership.organization_id,contextReady=performance.now();
    ...
    const modeDimension=mode==='customer'?'location':mode==='production'?'production_order':mode?'product':plan.dimension,runtimeWarning=queryResult.warning||plan.warning||null,visualization={type:mode==='matching'||mode==='customer'||mode==='production'?'table':mode==='forecast'||mode==='returns'?'bar':mode==='discount'?'curve':plan.visualization,title:plan.title,explanation:plan.explanation,metric:mode||plan.metric,dimension:modeDimension,rows:queryResult.data?.rows||[]},answer=queryResult.warning?`${plan.explanation} 현재 조건에서는 일부 데이터 조회가 제한되었습니다.`:`${plan.explanation} ${visualization.rows.length}개 결과를 현재 권한 범위에서 조회했습니다.${queryResult.data?.note?` ${queryResult.data.note}`:''}`;

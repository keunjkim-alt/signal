...
const pageAliases:any={'오늘의 액션':'action','오늘 결정할 일':'action','판매 현황':'hub','판매 허브':'hub','목표·마감 전망':'targets','목표·마감 예측':'targets','전사 목표·마감예측':'targets','수익성·할인':'profitability','수익성·할인 분석':'profitability','이상·기회 신호':'anomalies','지금 주목할 신호':'anomalies','판매 이상 감지':'anomalies','상품 수요·생애주기':'sales','제품 판매·생애주기':'sales','고객 인사이트':'customers','고객·지역':'customers','고객·지역 분석':'customers','반품·취소':'returns','반품·취소 분석':'returns','승인·결정 관리':'decisions','주요 의사결정':'decisions','경영 의사결정':'decisions','실행·성과':'execution','실행 성과':'execution','상품기획':'planning','디자인':'design','생산':'production','재고 운영':'inventory','마케팅':'marketing','마켓레이더':'market','디자이너 360':'designer','데이터 연결':'connections','데이터 연결 관리':'connections','사용자·권한':'permissions','사용자·권한 관리':'permissions'};
  ...
  try{
    ...
    for(let index=0;index<rows.length;index++){
      ...
      try{
        ...
        if(!['owner','admin'].includes(role)&&workspaceIds.length)await insert('workspace_memberships',workspaceIds.map(workspace_id=>({organization_id:org,workspace_id,membership_id:membership.id,status:'active'})),{upsert:true,onConflict:'workspace_id,membership_id'});
